...
- **Union** (A union B): the event that A or B (or both) occurs. In Lisp we compute it with `union`.
- **Intersection** (A intersection B): the event that both A and B occur. In Lisp we compute it with `intersection`.
- **Complement** (A^c or "not A"): the event that A does not occur. Its outcomes are those in Omega but not in A.
- **Difference** (A minus B): the event that A occurs but B does not. It equals A intersection (not B).
...
**Probability of the empty set is zero.** Since Omega and the empty set are disjoint and their union is Omega, by additivity P(Omega) + P(empty) = P(Omega), so P(empty) = 0.

**Probabilities are bounded above by 1.** Since A and its complement are disjoint and their union is Omega, we have P(A) + P(not A) = 1. Non-negativity of P(not A) forces P(A) <= 1. Combining with axiom 1, every probability lies in the interval [0, 1].

**Complement rule.** As already noted, P(not A) = 1 - P(A). This is often the fastest way to compute a probability when the direct calculation is awkward.

**Monotonicity.** If A is a subset of B, then P(A) <= P(B). This says that adding outcomes to an event can only make it more likely, which matches intuition.

**Inclusion-exclusion for two events.** For any events A and B:

    P(A union B) = P(A) + P(B) - P(A intersection B)

The term P(A intersection B) corrects for double-counting the outcomes that lie in both events. For three or more events, a longer inclusion-exclusion formula alternates signs; we will not need it in this book.
...
**Combinations** count unordered selections. The number of ways to choose k objects from n distinct objects without regard to order is the binomial coefficient C(n, k) = n! / (k! (n - k)!). We will meet the binomial coefficient again in the chapter on the binomial distribution.

For the two-dice sample space, no combinatorial machinery is needed: the sample space has exactly 6 * 6 = 36 elements, one for each ordered pair. But in a card problem, or in a problem involving many coin flips, you almost always start by counting the sample space with permutations or combinations.
...
**Problem 1.1.** For the two-dice sample space, compute P(sum is even) and P(sum is odd). Verify that they add to 1, as the axioms require.

**Problem 1.2.** Let A be the event "the first die is 3" and B be the event "the sum is 5." Compute P(A), P(B), P(A intersection B), and P(A union B) directly. Then verify the inclusion-exclusion formula P(A union B) = P(A) + P(B) - P(A intersection B).
...
**Problem 1.5.** Draw a card from a standard 52-card deck. Let A be the event "the card is a heart" and B be the event "the card is a face card (jack, queen, or king)." Assuming all 52 cards are equally likely, compute P(A), P(B), P(A intersection B), and P(A union B). Are A and B disjoint? Explain.

**Problem 1.6.** A fair coin is flipped 5 times. Using the classical definition, what is the probability of getting exactly 3 heads? Hint: the sample space has 2^5 = 32 outcomes, and the number of ways to arrange 3 heads among 5 flips is the binomial coefficient C(5, 3). Now compute the probability of "at least 3 heads" and the probability of "at most 3 heads," and use these to check the complement rule.

**Problem 1.7 (De Morgan in action).** For any two events A and B on any sample space, prove that P(not A intersection not B) = 1 - P(A union B). Then use this identity to give an alternative derivation of the inclusion-exclusion formula.

**Problem 1.8 (Coding exercise).** Modify the example program to accept an arbitrary predicate and print (a) the number of favorable outcomes, (b) the probability as an exact rational, and (c) the probability as a decimal. Then use this generalized reporter to compute P(the two dice differ by exactly 2).
